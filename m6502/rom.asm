 ...
 VLINE    PHA              ; SAVE ON STACK
          ...
          BCC   VLINEZ     ; NO, LOOP
 RTS1     RTS
 CLRSCR   LDY   #2Fh       ;MAX Y, FULL SCRN CLR
          BNE   CLRSC2     ;ALWAYS TAKEN
 CLRTOP   LDY   #27h       ;MAX Y, TOP SCREEN CLR
 CLRSC2   STY   V2         ;STORE AS BOTTOM COORD
                           ; FOR VLINE CALLS
          LDY   #27h       ;RIGHTMOST X-COORD (COLUMN)
 CLRSC3   LDA   #00h       ;TOP COORD FOR VLINE CALLS
 ...
 GBASCALC PHA              ;FOR INPUT 000DEFGH
          LSR
          AND   #03h
          ORA   #04h       ;  GENERATE GBASH=000001FG
          STA   GBASH
 ...
 NXTCOL   LDA   COLOR      ;INCREMENT COLOR BY 3
          CLC
          ADC   #03h
 SETCOL   AND   #0Fh       ;SETS COLOR=17*A MOD 16
          STA   COLOR
 ...
 IEVEN    LSR              ;LSB INTO CARRY FOR L/R TEST
          ...
          BNE   GETFMT
 ERR      LDY   #80h       ;SUBSTITUTE 80 FOR INVALID OPS
          LDA   #00h       ;SET PRINT FORMAT INDEX TO 0
 GETFMT   TAX
          LDA   FMT2,X     ;INDEX INTO PRINT FORMAT TABLE
          STA   FORMAT     ;SAVE FOR ADR FIELD FORMATTING
          AND   #03h       ;MASK FOR 2-BIT LENGTH
                           ; (P=1 BYTE, 1=2 BYTE, 2=3 BYTE)
          STA   LENGTH
          TYA              ;OPCODE
          AND   #8Fh       ;MASK FOR 1XXX1010 TEST
          TAX              ; SAVE IT
 ...
 PRNTOP   LDA   (PCL),Y
          JSR   PRBYTE
          LDX   #01h       ;PRINT 2 BLANKS
 PRNTBL   JSR   PRBL2
          CPY   LENGTH     ;PRINT INST (1-3 BYTES)
          INY              ;IN A 12 CHR FIELD
          BCC   PRNTOP
          LDX   #03h       ;CHAR COUNT FOR MNEMONIC PRINT
          CPY   #04h
          BCC   PRNTBL
 ...
 PRNTX    TXA              ;  OF BRANCH AND RETURN
          JMP   PRBYTE
 PRBLNK   LDX   #03h       ;BLANK COUNT
 PRBL2    LDA   #A0h       ;LOAD A SPACE
 ...
 *      (X=INDEX)
 ...
 1B A1 9D  344           DFB   1B,A1,9D,8A,1D,23
 9D 8B 1D  345           DFB   9D,8B,1D,A1,00,29
 19 AE 69  346           DFB   19,AE,69,A8,19,23
 24 53 1B  347           DFB   24,53,1B,23,24,53
 19 A1     348           DFB   19,A1    ;(A) FORMAT ABOVE
          DFB   00,1A,5B,5B,A5,69
 24 24     350           DFB   24,24    ;(B) FORMAT
          DFB   AE,AE,A8,AD,29,00
 7C 00     352           DFB   7C,00    ;(C) FORMAT
          DFB   15,9C,6D,9C,A5,69
 29 53     354           DFB   29,53    ;(D) FORMAT
          DFB   84,13,34,11,A5,69
 23 A0     356           DFB   23,A0    ;(E) FORMAT
 MNEMR    DFB   D8,62,5A,48,26,62
 94 88 54  358           DFB   94,88,54,44,C8,54
 68 44 E8  359           DFB   68,44,E8,94,00,B4
 08 84 74  360           DFB   08,84,74,B4,28,6E
 74 F4 CC  361           DFB   74,F4,CC,4A,72,F2
 A4 8A     362           DFB   A4,8A    ;(A) FORMAT
          DFB   00,AA,A2,A2,74,74
 74 72     364           DFB   74,72    ;(B) FORMAT
          DFB   44,68,B2,32,B2,00
 22 00     366           DFB   22,00    ;(C) FORMAT
          DFB   1A,1A,26,26,72,72
 88 C8     368           DFB   88,C8    ;(D) FORMAT
          DFB   C4,CA,26,48,44,44
 A2 C8     370           DFB   A2,C8    ;(E) FORMAT
          DFB   FF,FF,FF
